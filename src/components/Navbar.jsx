...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-center sticky top-0 p-4 bg-white shadow-sm items-center'>
       
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              <li className="bg-blue-500 text-white hover:bg-blue-600 font-medium">
          ...
          <ul className='flex flex-col gap-4 uppercase font-medium text-center'>
            {authState.isLoggedIn ? (
              <>
                ...
                <li className='py-2 px-3 cursor-pointer hover:bg-gray-200 transition' onClick={handleLogoutClick}>Logout</li>
              </>
            ) : (
